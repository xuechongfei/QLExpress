...
[source,java,indent=0]
----
include::./src/test/java/com/alibaba/qlexpress4/OperatorLimitTest.java[tag=operatorCheckStrategyExample]
----
...
[source,java,indent=0]
----
include::./src/test/java/com/alibaba/qlexpress4/Express4RunnerTest.java[tag=disableFunctionCallsExample]
----
 ...
 * `ImportManager.importClsAlias` assigns an alias to a class, **particularly suitable for code obfuscation scenarios**. This method directly uses the passed Class object instead of loading by class name, which avoids class name obfuscation after compilation

[source,java,indent=0]
----
include::./src/test/java/com/alibaba/qlexpress4/Express4RunnerTest.java[tag=importClsAlias]
----
...
[source,java,indent=0]
----
include::./src/test/java/com/alibaba/qlexpress4/ClearDfaCacheTest.java[tag=clearDFACacheBestPractice]
----
...
| SWITCH   | switch branch | Switch expression, case branch conditions and code blocks, and the default code block
...
The `*.` operator also supports automatic flattening of multi-level nested lists. When a list contains nested lists, the operator will automatically and recursively flatten all levels until it finds elements with the specified property or method:

[source,java]
----
include::./src/test/resources/testsuite/independent/spread/nested_list_spread.ql[tag=nestedSpreadExample]
----
...
Supports Java/C-like switch syntax:

[source,java]
----
include::./src/test/resources/testsuite/independent/doc/switch.ql[]
----

When switch is used as an expression, the -> arrow must be used (while the syntax above won't throw an error, the overall expression evaluates to null):

[source,java]
----
include::./src/test/resources/testsuite/independent/switch/switch_expression_basic.ql[tag=switchExpression]
----

Features:

* Uses `->` syntax, each case can return an expression value
* Supports multiple case values separated by commas (e.g., `case 90, 100 ->`)
* No fall-through, automatically exits after executing a case
* Can be used as an expression, assigned to variables or used in other expressions
